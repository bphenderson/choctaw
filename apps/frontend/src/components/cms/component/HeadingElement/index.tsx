 ...
 * 
 */
export const HeadingElementElement : CmsComponent<HeadingElementDataFragment, HeadingElementLayoutProps> = ({ contentLink, data, layoutProps, inEditMode, children, ctx }) =>
{
    if (isDefaultProps(layoutProps)) {
        return <CmsEditable as={DefaultHeadingElement} cmsId={ contentLink.key } data={ data } layoutProps={ layoutProps } ctx={ctx} />
    } 
    if (isAnimatedHeadingStylesProps(layoutProps)) {
        const withReducedMotion = inEditMode ? true : undefined
        return <CmsEditable as={AnimatedHeadingElement} cmsId={ contentLink.key } data={ data } layoutProps={ layoutProps } withReducedMotion={withReducedMotion} ctx={ctx} />
    }
    return <CmsEditable as="div" className="text-5xl p-4 md:p-8 xl:p-10" cmsId={ contentLink.key } ctx={ctx}>{ data?.headingText ?? ""}</CmsEditable>
}
