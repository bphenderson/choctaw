...
export const LocationPage: OptimizelyNextPage<
  LocationPageDataFragment
> = async ({
  contentLink,
  inEditMode,
  ctx,
      ...
      <div className="outer-padding">
        {image && (
          <div className="relative col-span-12 mt-8 md:mt-16 lg:mt-32 mb-8 lg:mb-24 mx-auto aspect-[1/1] md:aspect-[2/1] lg:aspect-[16/5] flex items-end">
            <CmsEditable
              cmsFieldName="LocationPostPromoImage"
              as={Image}
...
LocationPage.getDataFragment = () => [
  "LocationPageData",
  LocationPageDataFragmentDoc,
];
LocationPage.getMetaData = async (contentLink, locale, client) => {
  const sdk = getSdk(client);
  const result = await sdk.getLocationPageMetaData({
    key: contentLink.key,
    version: contentLink.version,
    locale: locale ? (localeToGraphLocale(locale) as Locales) : null,
  });
  const blogPost = (result.LocationPage?.pages || [])
  ...
  const meta: WithPropertySet<Metadata, "openGraph" | "other"> = {
    ...
    other: {
      "idio:content-type": "Location Page",
    },
  };
  const pageImage =
