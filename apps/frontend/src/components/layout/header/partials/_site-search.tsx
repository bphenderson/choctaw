...
export const SiteSearch: FunctionComponent<ConfiguredSiteSearchProps> = ({
  ...
  // Render search
  return (
    <div ref={container} className={"quick-search-container " + className}>
      {asSearchBox ? (
        <>
      ...
      {searchConfig.show_recent_searches &&
        searchBoxOpen &&
        !(isLoading || data) &&
        lastTerms && <LastSearchTerms lastTerms={lastTerms} />}
    </div>
  );
};
...
const LastSearchTerms: FunctionComponent<{ lastTerms?: Array<string> }> = ({
  ...
  return (
    <div className="quick-search-results border-solid border-2 border-azure dark:border-verdansk rounded-[1rem] bg-ghost-white dark:bg-vulcan z-[999] absolute w-full min-w-[25vw] min-h-[20px] top-16 right-0">
      <div className="px-2 pt-2 text-vulcan dark:text-ghost-white font-semibold">
      ...
      <div className="px-2 pb-2 text-xs italic text-vulcan-85 dark:text-ghost-white">
