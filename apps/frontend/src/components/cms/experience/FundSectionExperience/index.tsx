...
import {
  ...
  type FundSectionExperienceDataFragment,
...
import FundPostsSection from "./partials/fundposts";
import { getFundPosts } from "./actions/getFundPosts";
 ...
 */
export const FundSectionExperienceExperience: CmsComponent<
  FundSectionExperienceDataFragment
> = async ({ data, contentLink }) => {
  const composition = getFragmentData(
    CompositionDataFragmentDoc,
    getFragmentData(ExperienceDataFragmentDoc, data)?.composition,
  );
  const initialData = await getFundPosts({
    locale: contentLink.locale ?? "en",
    parentKey: contentLink.key ?? "n/a",
  });
  return (
    <div className="" data-component="FundSectionExperience">
...
FundSectionExperienceExperience.displayName =
  "Fund/News Section (Experience/FundSectionExperience)";
FundSectionExperienceExperience.getDataFragment = () => [
  "FundSectionExperienceData",
  FundSectionExperienceDataFragmentDoc,
];
FundSectionExperienceExperience.getMetaData = async (
  contentLink,
  locale,
  client,
) => {
  const sdk = getSdk(client);
  const data = await sdk.getFundSectionExperienceMetaData({
    key: contentLink.key,
  ...
  const imageData = getFragmentData(
    LinkDataFragmentDoc,
    getFragmentData(ReferenceDataFragmentDoc, seoData?.SharingImage)?.url,
  );
  const imageUrl = imageData?.default ?? undefined;
  const canonicalUrl = new URL(
    metaData?._metadata?.url?.default ?? "/",
    metaData?._metadata?.url?.base ?? "http://localhost:3000",
  );
  ...
  return {
    ...
    other: {
      "idio:content-type": "Fund Section (Experience)",
    },
  };
};
...
export default FundSectionExperienceExperience;
