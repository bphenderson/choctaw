...
export const VideoElementElement : CmsComponent<VideoElementDataFragment, VideoElementLayoutProps> = ({ data: {
    ...
    return <CmsEditable as={ Video } cmsId={ contentLink.key } src={ videoSrc } controls={ showControls == "true" } autoPlay={ autoPlay == "true" } muted={ muted == "true" || autoPlay == "true" } loop={ loop == "true" } className={ cssClasses.join(' ') } altText={ altText } placeholder={ placeholder } ctx={ctx} />
}
